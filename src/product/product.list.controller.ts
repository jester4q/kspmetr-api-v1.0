...
import { GetCategoryProductsQueryDto, GetCategoryProductsResponseDto } from './product.list.dto';
...
export class ProductListController {
    ...
    async list(@Param('categoryId') categoryId: number, @Query() query: GetCategoryProductsQueryDto): Promise<GetCategoryProductsResponseDto> {
        let category;
        const { reverse, depth, excludeCheckedToday } = query;
        try {
            category = await this.categoryService.fetchTree(categoryId);
        } catch (e) {
            throw new InternalServerErrorException('Culd not get category by id ' + categoryId);
        }
        if (!category) {
            throw new NotFoundException(`Category is not found by id ${categoryId}`);
        }
        const categoryIds = this.get3LevelCategories(category);
        if (categoryIds.length) {
            //try {
            const products = await this.productService.fetchAllInCategory(categoryIds, depth, reverse, excludeCheckedToday);
            return { items: products };
            //} catch (e) {
            //    throw new InternalServerErrorException('Culd not get products of the category');
            //}
        }
        return { items: [] };
    }
